...
export const TimeSelect = React.createClass({
    ...
    timearrInit:function(next){
        //设置时间可选值数组
        let maxhour = parseInt(next.maxhour)  || 60;
        let minhour = parseInt(next.minhour)  || 0;
    ...
    componentWillReceiveProps: function(next) {
        ...
        if(next.hourstep!=this.props.hourstep || next.minhour!=this.props.minhour
            || next.maxhour!=this.props.maxhour || next.ArrayInit===true || next.needUpdateArray == true){
        ...
        if(next.defaulthour!==undefined && this.props.show !==undefined ){
            if (this.props.show ===false && next.show === true) {
                let hourarr = next.hourarray;
                let defaultHour = next.defaulthour;
                let index = hourarr.indexOf(defaultHour);
                if (index != -1) {
                    this.setState({
                        hourtime: next.defaulthour,
                        hourindex: index
    ...
    render: function() {
        ...
        let selecttitle = this.props.title || '';
        let statusshow = this.props.statusshow || false;
        ...
        let hourunit = this.props.hourunit || '';
        let minuteunit = this.props.minuteunit || '';
        minuteindex = hourindex ==14 ? 0:minuteindex;


        let hourseAppendStyle =  {top:hourtop+'%',left:minuteshow?25+'%':45+'%',display:hourshow?'block':'none'};
        ...
        return (
            <section ref="selecter" style={{display:this.state.timeDisplay?'block':'none',opacity:this.state.showOpacity}} className='timeSelect'>
                ...
                <section className="timeselect" onTouchMove={this.endDefault}>
                    ...
                    <section className='time'>
                        ...
                        <section className='timetext'>
                            <span style={{left:4+'%'}} className={statusshow?'status show':'status'}>{''}</span>
                            <span className='hour' style={{left:minuteshow?33+'%':56+'%',display:hourshow?'inline-block':'none'}}>{hourunit}</span>
                            <span className='minute' style={{display:minuteshow?'inline-block':'none',left:hourshow?66+'%':53+'%'}}>{minuteunit}</span>
                            <span className={statusshow?'status show':'status'}>{''||statusname}</span>
                        </section>
                        <section className='hourvalue flex-column' style={hourseAppendStyle}>
                        ...
                        <section  className='minutevalue flex-column' style={{top:minutetop+'%',display:minuteshow?'block':'none',left:hourshow?58+'%':45+'%'}}>
