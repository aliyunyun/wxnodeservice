 ...
 * @prop {boolean} loop 是否需要循环滑动选择(默认值为false)
 * @author   xinglin
 * @updateTime 2017-04-26
 */
...
export const TimeSelect = React.createClass({
	...
	endrange:function(e){
		...
		if(newy-oldy>20 && type=='hour'){
			let rangestep = parseInt((newy-oldy)/35)>0 ? parseInt((newy-oldy)/35) : 1;
			hourindex = hourindex-rangestep;
			if(this.props.loop){
				hourindex = hourindex<0 ?
							(hourarr.length + hourindex % hourarr.length) :
							hourindex;
			}else{
		...
		//小时增加
		if(newy-oldy<-20 && type=='hour'){
			let rangestep = parseInt((oldy-newy)/35)>0 ? parseInt((oldy-newy)/35) : 1;
			hourindex = hourindex+rangestep;
			if(this.props.loop){
				hourindex = (hourindex>=hourarr.length) ?
						(hourindex-Math.floor(hourindex/hourarr.length)*hourarr.length):
						hourindex;
			}else{
				hourindex = (hourindex>=hourarr.length) ?
						(hourarr.length-1):
		...
		if(newy-oldy>20 && type=='minute'){
			...
			if(this.props.loop){
				minuteindex = minuteindex<0 ? minutearr.length+minuteindex % minutearr.length : minuteindex;
			}else{
		...
		if(newy-oldy<-20 && type=='minute'){
			...
			minuteindex = minuteindex+rangestep;
			if(this.props.loop){
				minuteindex = (minuteindex>=minutearr.length)?
						  (minuteindex-Math.floor(minuteindex/minutearr.length)*minutearr.length):
						  minuteindex;
			}else{
				minuteindex = (minuteindex>=minutearr.length)?
						  (minutearr.length-1):
	...
	render: function() {
		...
		let loop = this.props.loop || false;
		...
		let hLen = hourarr.length,
			mLen = minutearr.length;
	    return (
		        ...
		        <section className="timeselect" onTouchMove={this.endDefault}>
		        	...
		        	<section className='time'>
		        		...
		        		<section className='hourvalue flex-column' style={{top:hourtop+'%',left:minuteshow?25+'%':45+'%',display:hourshow?'':'none'}}>
		        			<span className={(hourindex-3)<0&&!loop ? "line4" : "line1"}>{(hourindex-3)<0?hourarr[hLen+hourindex-3]:hourarr[hourindex-3]}</span>
		        			<span className={(hourindex-2)<0&&!loop ? "line4" : "line1"}>{(hourindex-2)<0?hourarr[hLen+hourindex-2]:hourarr[hourindex-2]}</span>
		        			<span className={(hourindex-1)<0&&!loop ? "line4" : "line2"}>{(hourindex-1)<0?hourarr[hLen+hourindex-1]:hourarr[hourindex-1]}</span>
		        			<span className='line3'>{hourarr[hourindex]}</span>
		        			<span className={(hourindex+1)>=hLen&&!loop ? "line4" : "line2"}>{(hourindex+1)>=hLen?hourarr[hourindex+1-hLen]:hourarr[hourindex+1]}</span>
		        			<span className={(hourindex+2)>=hLen&&!loop ? "line4" : "line1"}>{(hourindex+2)>=hLen?hourarr[hourindex+2-hLen]:hourarr[hourindex+2]}</span>
		        			<span className={(hourindex+3)>=hLen&&!loop ? "line4" : "line1"}>{(hourindex+3)>=hLen?hourarr[hourindex+3-hLen]:hourarr[hourindex+3]}</span>
		        		</section>
		        		<section  className='minutevalue flex-column' style={{top:minutetop+'%',display:minuteshow?'':'none',left:hourshow?58+'%':45+'%'}}>
			        		<span className={(minuteindex-3)<0&&!loop ? "line4" : "line1"}>{(minuteindex-3)<0?minutearr[mLen+minuteindex-3]:minutearr[minuteindex-3]}</span>
		        			<span className={(minuteindex-2)<0&&!loop ? "line4" : "line1"}>{(minuteindex-2)<0?minutearr[mLen+minuteindex-2]:minutearr[minuteindex-2]}</span>
		        			<span className={(minuteindex-1)<0&&!loop ? "line4" : "line2"}>{(minuteindex-1)<0?minutearr[mLen+minuteindex-1]:minutearr[minuteindex-1]}</span>
		        			<span className='line3'>{minutearr[minuteindex]}</span>
		        			<span className={(minuteindex+1)>=mLen&&!loop ? "line4" : "line2"}>{(minuteindex+1)>=mLen?minutearr[minuteindex+1-mLen]:minutearr[minuteindex+1]}</span>
		        			<span className={(minuteindex+2)>=mLen&&!loop ? "line4" : "line1"}>{(minuteindex+2)>=mLen?minutearr[minuteindex+2-mLen]:minutearr[minuteindex+2]}</span>
		        			<span className={(minuteindex+3)>=mLen&&!loop ? "line4" : "line1"}>{(minuteindex+3)>=mLen?minutearr[minuteindex+3-mLen]:minutearr[minuteindex+3]}</span>
		        		</section>
		        	</section>
