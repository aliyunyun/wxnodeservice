			...
			<div className='page page_tags'>
            	...
            	<header className='header'>
            		<div className='panel'>
            			{<div className='tag_wrap' id='tagWrap' style={{display:showMore?'none':'block'}}>
            				<div style={{display:'inline-block', paddingRight:'3.333rem'}}>
	                    	{
	                    		this.state.panels.map((item,i)=>{
	                    			...
	                    			return (<div key={name} className={'tag_item '+(this.state.currTag==name?'selected':'')}  onTouchTap={this.getAlbums.bind(this,item)}>
	                    					<span>{name}</span>
	                    				</div>)
	                    		})
	                    	}
                    	...
                    	<span className='more' onTouchEnd={this.toggle.bind(this)}>
                    		<i className={'arrow_bottom '+(showMore?'roate_top':'roate_bottom')}></i>
                    	</span>
                    </div>
            	</header>
